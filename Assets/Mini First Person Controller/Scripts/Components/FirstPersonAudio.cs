...
public class FirstPersonAudio : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        {
            volume = float.Parse(PlayerPrefs.GetString("SETTINGS").Split(" ")[2]);
        }
        stepAudio.volume *= volume;
        runningAudio.volume *= volume;
        landingAudio.volume *= volume;
        jumpAudio.volume *= volume;
        crouchStartAudio.volume *= volume;
        crouchedAudio.volume *= volume;
        crouchEndAudio.volume *= volume;
        for (int i = 0; i < MovingAudios.Length; i++)
        {
            MovingAudios[i].volume *= volume;
        }
    }
